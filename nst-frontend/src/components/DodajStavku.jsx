...
import { dodajStavku, getProizvod, getProizvodi } from '../services/Service';
...
const DodajStavku = () => {
  ...
  //uzima broj narudzbenice iz url-a
  ...
  const handleSubmit = async (e) => {
    ...
    if(!imaGresaka) {
      try {
        await dodajStavku(stavka); 
        console.log('Stavka uspešno dodata:', stavka);
        navigator(`/prikazi-narudzbenicu/${id}`);
      } catch (error) {
    ...
    <div className="card p-4" style={{ maxWidth: '600px', width: '100%' }}>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="d-flex justify-content-between">
          ...
          <Link className="btn btn-danger" to={`/prikazi-narudzbenicu/${id}`}>Cancel</Link>
        </div>
      </form>
    </div>
  </div>
